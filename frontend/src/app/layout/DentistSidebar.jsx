...
export function DentistSidebar({ isMobileOpen, onClose }) {
  const [isCollapsed, setIsCollapsed] = useState(false);
  const user = useAuthStore((state) => state.user);
  const { activePatientId } = useDentistStore();
  ...
  // Custom Dentist Menu structure with dynamic contextual routing
  const getMenuItemPath = (key) => {
    if (key === 'dashboard') return '/dentist/dashboard';
    if (key === 'appointments') return '/appointments/calendar';
    if (key === 'patients') return '/dentist/patients';
    
    // Scoped sub-tabs of patient detail page
    if (!activePatientId) return null;
    return `/dentist/patients/${activePatientId}?tab=${key}`;
  };
  ...
  const handleContextClick = (item, e) => {
    if (item.key !== 'dashboard' && item.key !== 'appointments' && item.key !== 'patients' && !activePatientId) {
      e.preventDefault();
      ...
      >
        ...
        <nav className="flex-1 px-3 py-2 space-y-1 overflow-y-auto no-scrollbar">
          {menuItems.map((item) => {
            ...
            const targetPath = getMenuItemPath(item.key) || '/dentist/patients';
            ...
            const isAppointmentsActive = item.key === 'appointments' && location.pathname === '/appointments/calendar';
            ...
            <div className="flex items-center gap-3 text-left w-full overflow-hidden">
              ...
              <div className="overflow-hidden flex-1 min-w-0 flex flex-col justify-center">
                ...
                <p className="text-[9px] text-muted-foreground truncate font-semibold uppercase tracking-wider mt-0.5 leading-none">
                  {user.role ? user.role.replace('_', ' ') : 'Dentist'}
                </p>
              </div>
            </div>
          )}
