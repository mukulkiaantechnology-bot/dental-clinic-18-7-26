...
export function LabSidebar({ isMobileOpen, onClose }) {
  ...
  const menuItems = [
    { key: 'cases', label: 'Lab Cases', icon: 'Folder', path: '/lab/cases' },
    { key: 'status', label: 'Status Board', icon: 'Columns', path: '/lab/status' }
  ];
  ...
  return (
    <>
      ...
      >
        ...
        {/* Active Session Info — role + name only, no case tag */}
        {(!isCollapsed || isMobileOpen) ? (
          ...
          <div className="my-4 flex justify-center" title="Lab Coordinator Active">
            ...
            <div className="flex items-center gap-3 text-left w-full overflow-hidden">
              ...
              <div className="overflow-hidden flex-1 min-w-0 flex flex-col justify-center">
                ...
                <p className="text-[9px] text-muted-foreground truncate font-semibold uppercase tracking-wider mt-0.5 leading-none">
                  {user.role ? user.role.replace('_', ' ') : 'Lab Coordinator'}
                </p>
              </div>
            </div>
          )}
